...
def plot_kryptonite(
    ...
    (
        df[df.Kryptonite_type == "Gold"].plot(
            kind="scatter",
            color="goldenrod",
            marker="s",
            s=30,
    ...
    (
        df[df.Kryptonite_type == "Red"].plot(
            kind="scatter",
            color="darkred",
            marker="x",
            s=25,
...
date_series = pd.Series(
    [-10, 1, 2, np.nan, 4],
    index=DTI(["2001-01-01", "2001-01-05", "2001-01-10", "2001-02-01", "2001-02-05"]),
)
...
def plot_filled_trend(s: pd.DataFrame):
    ...
    plt.xticks(
        range(n), labels=["Actual", "Actual", "Actual", "Imputed (Feb 1)", "Actual"]
    )
    plt.plot([3], [3.69], "x", label="Time interpolated value")
    plt.legend()
...
def make_cars(fname="data/cars/car.data"):
    ...
    cars["passengers"] = cars.passengers.map({"2": 2, "4": 4, "more": 6})
    cars["trunk"] = cars.trunk.map({"small": 0, "med": 1, "big": 2})
...
def make_ssa_synthetic(fname="data/Baby-Names-SSA.csv"):
    ...
    flowers = ("Daisy", "Orchid", "Rose", "Violet", "Lily")
    ...
    for (rank_gender, year), name in unstack.iteritems():
        ...
        age = now - year
        count = int(year_to_pop[year] * rank_to_freq[rank])
        for _ in range(count):
            color = random.choice(colors)
            flower = random.choice(flowers)
            rows.append((age, gender, name, color, flower))

    df = pd.DataFrame(rows)
    df.columns = ("Age", "Gender", "Name", "Favorite_Color", "Favorite_Flower")
...
def make_birth_month(ages):
    ...
    for age in ages:
        ...
        probs[0] += bias
        probs /= np.sum(probs)
        birth.append(choice(month_names, 1, p=probs)[0])
    return birth


def make_ssa_states(names="data/usa_names_all.parq", pops="data/state-population.fwf"):
    np.random.seed(1)
    choice = np.random.choice
    ...
    names = names[cols]
    names.to_parquet("data/usa_names_states.parq", index=False)
...
def make_unscaled_features(N=200, j1=1 / 50, j2=1 / 10):
    ...
    df = pd.DataFrame({"Feature_1": feat1, "Feature_2": feat2, "Target": target})
    return df.sample(frac=1).reset_index().drop("index", axis=1)
...
def plot_digits(data, digits, decomp="Unknown", colors=grays10):
...
class requests:
    "Just barely mock requests library enough to run cells"

    @staticmethod
    def get(url):
        Result = namedtuple("Result", "status_code headers text url")
        return Result(
            status_code=200,
            headers={"Content-Type": "application/json; charset=utf-8"},
            text='{"1":{"name":"Guido van Rossum","password":"unladenswallow",'
            '"details":{"profession":"ex-BDFL"}},'
            '"2":{"name":"Brendan Eich","password":"nontransitiveequality",'
            '"details":{"profession":"Mozillan"}},'
            '"3":{"name":"Ken Thompson","password":"p/q2-q4!",'
...
def make_mongo_biz(client=connect_mongo()):
    ...
    for n in range(50):
        ...
        name = f"{choice(words)} {choice(words)} " f"{choice(title)}"
        info[name] = random_phone()
        biz = {"name": name, "cuisine": choice(cuisine), "phone": info[name]}
